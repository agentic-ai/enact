...
def _add_fast_api(
  ...
  def invoke() -> Optional[resources.Resource]:
    with store:
      return invokable.invoke().get_output()

  def invoke_input(input: resources.Resource) -> Optional[resources.Resource]:
    with store:
      return invokable.invoke(enact.commit(input)).get_output()

  wrapper_fn = (invoke if invokable.get_input_type() == enact.NoneResource
                else invoke_input)
  app.router.add_api_route(
    route, wrapper_fn, name=type(invokable).__name__,
